...
void initialize_default_configs() {
    if (!SPIFFS.exists(CONFIG_FILE)) {
        ...
        for (int i = 0; i < MAX_CONFIGS; i++) {
            ...
            memset(configs[i].transpose_array, 0x00, 4);
        }
...
bool store_active_config(int active_config) {
    ...
    if (!file) file = SPIFFS.open(CONFIG_FILE, "w+");
...
void store_config(const char name[10], int config_num, uint8_t wheel_id, const uint8_t transpose[4]) {
    ...
    memcpy(new_config.transpose_array, transpose, 4);
        ...
        for (int i = 0; i < MAX_CONFIGS; i++) {
            if (configs[i].config_number == config_num) {
                ...
                *wheel_id = configs[i].wheel_ident;
                memcpy(transpose_out, configs[i].transpose_array, 4);
                return true;
            }
        }
    }
